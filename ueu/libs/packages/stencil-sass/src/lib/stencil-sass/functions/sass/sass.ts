import { loadDiagnostic } from '../diagnostics/diagnostics';
import { createResultsId, getRenderOptions, usePlugin } from '../utils';
...
export function sass(opts: PluginOptions = {}): Plugin {
    return {
        ...
        transform: (sourceText: string, fileName: string, context: PluginCtx) => {
            /** Checks if it's a valid file and if the sourceText if valid */
